...
class NotaDbHelper {
  ...
  Future<Database> get database async{
    if(_database != null){
      return _database!;  // se já houver conexão retorna a que já foi criada
    }
    _database = await _initDatabase();  // caso não haja conexão retorna uma nova
    return _database!;
  }
  ...
  Future<Database> _initDatabase() async{
    ...
    return await openDatabase(
      ...
      version: 1,  // implementar o upgrade version()
    );
  }
  ...
  Future<int> insertNota(Nota nota) async{
    ...
    return db.insert(TABLE_NAME, nota.toMap());  // converte a nova para modelo e DB
  }
  //* read
  ...
  Future<int> updateNota(Nota nota) async{
    ...
    final db = await database;
    return await db.update(TABLE_NAME, nota.toMap(), where: "id = ?", whereArgs: [nota.id]);
  }
  //* delete
